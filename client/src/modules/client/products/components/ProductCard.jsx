...
export default function ProductCard({ product }) {
  ...
  return (
    <Link 
      to={productUrl}
      className="group bg-white rounded-xl border border-gray-100 flex flex-col overflow-hidden w-full no-underline text-inherit hover:border-gray-300 transition-all duration-200"
    >
      {/* ================= KHU VỰC 1: HÌNH ẢNH ================= */}
      <div className="relative aspect-square bg-white p-4 flex items-center justify-center">
        <img 
          src={thumbnail} 
          alt={name} 
          className={`max-w-full max-h-full object-contain transition-transform duration-300 group-hover:scale-102 ${isOutOfStock ? 'opacity-40' : ''}`} 
        />
        
        {/* Nhãn giảm giá phẳng góc trái */}
        {hasDiscount && (
          <span className="absolute top-2 left-2 bg-orange-500 text-white text-[11px] font-bold px-1.5 py-0.5 rounded">
            -{discountPercent}%
          </span>
        )}

        {/* Cột thông số kĩ thuật nhỏ đè lên góc phải của ảnh giống hệt ảnh mẫu */}
        {specs && specs.length > 0 && (
          <div className="absolute top-2 right-2 flex flex-col gap-1 items-end">
            {specs.slice(0, 3).map((spec, index) => (
              <span key={index} className="bg-gray-50 border border-gray-100 text-gray-500 text-[10px] px-1.5 py-0.5 rounded font-medium">
                {spec}
              </span>
            ))}
          </div>
      ...
      {/* ================= KHU VỰC 2: THÔNG TIN ================= */}
      <div className="p-3.5 flex flex-col flex-1 gap-1.5 border-t border-gray-50">
        
        {/* Tên sản phẩm */}
        <h3 className="text-[14px] font-medium text-gray-800 line-clamp-2 leading-snug h-[40px] m-0 group-hover:text-blue-600 transition-colors">
          {name}
        </h3>
