    ...
    <div className="flex flex-col gap-4">
          ...
          {itemsList.map((item) => (
            <div key={item.id} className="p-4 flex gap-4 items-center hover:bg-gray-50/50 transition-colors">
              ...
              <div className="flex-1">
                <p className="font-semibold text-gray-800 text-sm line-clamp-2">{item.productName}</p>
                <p className="text-xs text-gray-500 mt-1 bg-gray-100 inline-block px-2 py-0.5 rounded">
                  {item.option1Value && `${item.option1Name}: ${item.option1Value}`} 
                  {item.option2Value && ` | ${item.option2Name}: ${item.option2Value}`}
                </p>
              </div>
              ...
              <div className="w-28 text-right font-bold text-blue-600">
      ...
      {activeAction === 'PACK' && (
        <div className="bg-indigo-50/80 p-5 rounded-xl border border-indigo-200 shadow-inner animate-in fade-in slide-in-from-bottom-2">
          ...
          <div className="flex flex-col gap-4">
            {itemsList.map((item) => {
              // Hỗ trợ cả 2 tên field: productVariantId (trong DB) hoặc variantId (trong API trả về)
              const vId = item.productVariantId || item.variantId || item.id; 
              ...
              return (
                <div key={vId} className={`flex flex-col gap-2 p-4 rounded-lg shadow-sm border ${isRequireSerial ? 'bg-white border-blue-200' : 'bg-gray-50 border-gray-200 opacity-80'}`}>
                  <div className="flex justify-between items-center">
                    ...
                    <span className={`text-xs font-bold px-2 py-1 rounded ${isRequireSerial ? 'text-blue-600 bg-blue-100' : 'text-gray-600 bg-gray-200'}`}>
                  ...
                  {isRequireSerial ? (
                    <Select
                      ...
                      // BỎ open={false} đi để nó hoạt động bình thường như thẻ input tags
                      tokenSeparators={[',', '\n', ' ']} // Nhận diện dấu phẩy, khoảng trắng, Enter để cắt tags
                      value={packData.serials ? packData.serials[vId] : []}
                      onChange={(values) => {
                        if (values.length > item.quantity) {
                          message.error(`❌ Khách chỉ đặt ${item.quantity} cái! Không được quét thừa!`);
                          return;
                        }
                        setPackData({
                          ...packData,
                      ...
                      // Fix lỗi không gõ được: Cho phép tự do nhập giá trị không cần list option
