...
const OrderManagement = () => {
  ...
  // ==========================================================
  // TÍNH TOÁN SỐ LIỆU CHO 4 THẺ THỐNG KÊ (STAT CARDS)
  // ==========================================================
  const stats = useMemo(() => {
  ...
  return (
    // GIẢM KHOẢNG CÁCH GAP VÀ PADDING Ở CONTAINER CHÍNH
      ...
      <div className="bg-white p-3 rounded-xl border border-gray-100 shadow-sm flex flex-col xl:flex-row justify-between items-start xl:items-center gap-3">
        ...
        <div className="flex items-center gap-2 w-full xl:w-auto">
          ...
          <button 
            ...
            disabled={isPrinting}
            className="flex items-center gap-1.5 px-3 py-2 bg-blue-50 text-blue-600 border border-blue-200 hover:bg-blue-600 hover:text-white rounded-lg text-[13px] font-bold transition-all shrink-0 shadow-sm disabled:opacity-50 disabled:cursor-wait"
          >
            {isPrinting ? <Loader2 size={15} className="animate-spin" /> : <Printer size={15} />}
            <span>In Phiếu {selectedRowKeys.length > 0 ? `(${selectedRowKeys.length})` : '(Tất cả)'}</span>
          </button>
