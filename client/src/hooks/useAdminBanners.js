...
export const useAdminBanners = (page = 0, size = 10) => {
  ...
  const createBanner = useMutation({
    ...
    onError: (err) => {
      ...
      toast.error(errorData?.messages || 'Lỗi khi thêm banner');
    },
  });
  ...
  const updateBanner = useMutation({
    ...
    onError: (err) => {
      const errorData = err.response?.data;
      if (errorData && errorData.errors) return;
      toast.error(errorData?.messages || 'Lỗi khi cập nhật banner');
    },
  });
  ...
  const deleteBanner = useMutation({
    ...
    onError: (err) => {
      toast.error(err.response?.data?.messages || 'Lỗi khi xóa banner');
    }
  });
