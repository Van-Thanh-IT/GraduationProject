...
const getPlaceholder = (optionName) => {
  const map = {
    "Màu sắc": "VD: Đỏ, Xanh, Đen",
    "Lựa chọn phiên bản": "VD: V1, V2, Pro",
    "Dung lượng": "VD: 128GB, 256GB, 512GB",
    "Kết nối mạng": "VD: Wifi, 5G",
    "RAM": "VD: 8GB, 16GB",
    "Card đồ họa": "VD: RTX 4060, RTX 4070",
    "Kích thước mặt": "VD: 40mm, 44mm",
    "Loại dây": "VD: Da, Thép, Silicon",
    "Mặc Định": "Nhập giá trị"
  };

  return map[optionName] || "Nhập thông tin";
};
        ...
        {variants.map((variant, index) => (
          <div key={index} className="flex items-start gap-3 p-3 bg-white border border-slate-200 rounded-xl shadow-sm relative group">
            ...
            <div className="flex-1 grid grid-cols-2 lg:grid-cols-5 gap-3">
              {hasOpt1 && (
                <Input label={product.option1Name} required placeholder={getPlaceholder(product.option1Name)} value={variant.option1Value} onChange={e => handleChange(index, 'option1Value', e.target.value)} />
              )}
              {hasOpt2 && (
                <Input label={product.option2Name} required placeholder={getPlaceholder(product.option2Name)} value={variant.option2Value} onChange={e => handleChange(index, 'option2Value', e.target.value)} />
              )}
              {hasOpt3 && (
                <Input label={product.option3Name} required placeholder={getPlaceholder(product.option3Name)} value={variant.option3Value} onChange={e => handleChange(index, 'option3Value', e.target.value)} />
              )}
