        ...
        try {
            ...
            if (dataArray.isArray()) {
                for (JsonNode rateNode : dataArray) {
                    ...
                    long totalFee = rateNode.path("total_fee").asLong(0);
                    long insuranceFee = rateNode.path("insurrance_fee").asLong(0);

                    long actualShippingFee = totalFee - insuranceFee;

                    if (actualShippingFee < 0) actualShippingFee = 0;

                    rateMap.put("total_fee", actualShippingFee);

                    rateMap.put("original_total_fee", totalFee);
                    rateMap.put("shop_paid_insurance", insuranceFee);
