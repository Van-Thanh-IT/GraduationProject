...
public class ProductService {
    ...
    public AiProductResult findBestProductContextForAI(AiSearchCriteria criteria) {
        ...
        // Trường hợp 1: Khách nói "khoảng X" (AI set min = max = X)
        if (finalMin != null && finalMax != null && finalMin.compareTo(finalMax) == 0) {
            BigDecimal margin = finalMax.multiply(new BigDecimal("0.20"));
            finalMin = finalMin.subtract(margin);
            finalMax = finalMax.add(margin);
        }
        // Trường hợp 2: Khách nói "dưới X", "tầm X đổ lại" (AI set max = X, min = null)
        else if (finalMax != null && finalMin == null) {
            BigDecimal margin = finalMax.multiply(new BigDecimal("0.05"));
            finalMax = finalMax.add(margin);
            finalMin = null;
        }
        // Trường hợp 3: Khách nói "trên X", "từ X trở lên" (AI set min = X, max = null)
        else if (finalMin != null && finalMax == null) {
            BigDecimal margin = finalMin.multiply(new BigDecimal("0.05"));
            finalMin = finalMin.subtract(margin);
            finalMax = null;
        }
        // Trường hợp 4: Khách cho khoảng "từ X đến Y"
        else if (finalMin != null && finalMax != null) {
            BigDecimal marginMin = finalMin.multiply(new BigDecimal("0.05"));
            BigDecimal marginMax = finalMax.multiply(new BigDecimal("0.05"));
            finalMin = finalMin.subtract(marginMin);
            finalMax = finalMax.add(marginMax);
        }
