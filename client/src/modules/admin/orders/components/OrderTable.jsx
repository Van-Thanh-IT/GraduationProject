...
const DownloadVatButton = ({ orderId, onDownloadInvoice }) => {
  ...
  return (
    <Button
      ...
      loading={isLoading}
      icon={!isLoading && <FileDown size={14} className="group-hover:-translate-y-0.5 transition-transform" />}
      className={`ml-auto flex items-center gap-1.5 px-2.5 py-1 bg-white border text-emerald-600 rounded-md transition-all shadow-sm group h-auto ${
    ...
    },
    {
      ...
      render: (_, row) => (
        <Button
          type="text"
          onClick={() => onView(row)}
          icon={<Eye size={16} className="group-hover:scale-110 transition-transform" />}
          className="mx-auto flex items-center justify-center w-8 h-8 text-blue-600 hover:text-white hover:bg-blue-600 rounded-lg transition-all shadow-sm border border-blue-100 bg-blue-50/50 group p-0"
          title="Xem chi tiết"
        />
      ),
    },
